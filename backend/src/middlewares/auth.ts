...
export function ensureAuth(req:AuthenticatedRequest , res: Response, next: NextFunction){
    ...
    if(!authorizationHeader) return res.status(401).json({message:"Usuário não autorizado : Nenhum token encontrado"})
    //Bearer qw0iqujfkafapew

     const token = authorizationHeader.replace(/Bearer/,"")

     jwtService.verifyToken(token, async(err,decoded) =>{
        if(err || typeof decoded === "undefined") return res.status(401).json({message: "Usuário não autorizado : Token inválido"})
